...
- [ ] كل endpoint API جديد يمر عبر `requireTenantSession()` — لا يوجد وصول بدون مصادقة
- [ ] كل بيانات Tenant معزولة بـ `tenantId` filter
- [ ] Custom domains يتم التحقق من ملكيتها بـ DNS verification
- [ ] Webhooks تُرسل مع HMAC signature للتحقق
- [ ] الصلاحيات المخصصة لا تسمح بتصعيد الامتيازات
- [ ] كل التقارير محددة بنطاق الـ Tenant فقط
