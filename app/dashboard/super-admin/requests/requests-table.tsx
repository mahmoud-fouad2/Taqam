...
export function RequestsTable() {
  ...
  const [statusFilter, setStatusFilter] = React.useState<SubscriptionRequest["status"] | "all">(
    "all"
  );
  const [sortBy, setSortBy] = React.useState<
    "newest" | "oldest" | "employeesDesc" | "employeesAsc"
  >("newest");
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center">
        <XCircle className="text-destructive mb-3 h-10 w-10" />
      ...
      <div className="space-y-3 border-b p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="default">
            {t.common.accepted}: {totals.approved}
          </Badge>
          <Badge variant="destructive">
            {t.common.rejected}: {totals.rejected}
          </Badge>
        </div>
        ...
        <div className="flex flex-col gap-2 lg:flex-row">
          ...
          <select
            ...
            value={statusFilter}
            onChange={(event) =>
              setStatusFilter(event.target.value as SubscriptionRequest["status"] | "all")
            }>
          ...
          <select
            ...
            onChange={(event) =>
              setSortBy(
                event.target.value as "newest" | "oldest" | "employeesDesc" | "employeesAsc"
              )
            }>
