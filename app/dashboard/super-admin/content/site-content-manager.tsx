    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="siteNameAr">{isAr ? "اسم المنصة بالعربية" : "Arabic site name"}</Label>
              <Input id="siteNameAr" value={form.siteNameAr} onChange={(event) => updateField("siteNameAr", event.target.value)} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="siteNameEn">{isAr ? "اسم المنصة بالإنجليزية" : "English site name"}</Label>
              <Input id="siteNameEn" value={form.siteNameEn} onChange={(event) => updateField("siteNameEn", event.target.value)} />
            </div>
          </div>
          ...
          <Tabs defaultValue="home" className="w-full">
            ...
            <TabsContent value="home" className="space-y-4 pt-4">
              <LocalizedField labelAr="شارة القسم" labelEn="Badge" valueAr={form.home.badge.ar} valueEn={form.home.badge.en} onChangeAr={(value) => updateLocalizedSection("home", "badge", "ar", value)} onChangeEn={(value) => updateLocalizedSection("home", "badge", "en", value)} />
              <LocalizedField labelAr="العنوان الرئيسي" labelEn="Hero title" valueAr={form.home.title.ar} valueEn={form.home.title.en} onChangeAr={(value) => updateLocalizedSection("home", "title", "ar", value)} onChangeEn={(value) => updateLocalizedSection("home", "title", "en", value)} textarea />
              <LocalizedField labelAr="الوصف" labelEn="Description" valueAr={form.home.description.ar} valueEn={form.home.description.en} onChangeAr={(value) => updateLocalizedSection("home", "description", "ar", value)} onChangeEn={(value) => updateLocalizedSection("home", "description", "en", value)} textarea />
              <LocalizedField labelAr="نص الزر الأساسي" labelEn="Primary CTA label" valueAr={form.home.primaryCtaLabel.ar} valueEn={form.home.primaryCtaLabel.en} onChangeAr={(value) => updateLocalizedSection("home", "primaryCtaLabel", "ar", value)} onChangeEn={(value) => updateLocalizedSection("home", "primaryCtaLabel", "en", value)} />
              <div className="space-y-2">
                <Label htmlFor="homeCtaHref">{isAr ? "رابط الزر الأساسي" : "Primary CTA href"}</Label>
                <Input id="homeCtaHref" value={form.home.primaryCtaHref} onChange={(event) => updateField("home", { ...form.home, primaryCtaHref: event.target.value })} />
              </div>
            </TabsContent>

            <TabsContent value="pricing" className="space-y-4 pt-4">
              <LocalizedField labelAr="شارة القسم" labelEn="Badge" valueAr={form.pricing.badge.ar} valueEn={form.pricing.badge.en} onChangeAr={(value) => updateLocalizedSection("pricing", "badge", "ar", value)} onChangeEn={(value) => updateLocalizedSection("pricing", "badge", "en", value)} />
              <LocalizedField labelAr="العنوان" labelEn="Title" valueAr={form.pricing.title.ar} valueEn={form.pricing.title.en} onChangeAr={(value) => updateLocalizedSection("pricing", "title", "ar", value)} onChangeEn={(value) => updateLocalizedSection("pricing", "title", "en", value)} />
              <LocalizedField labelAr="الوصف" labelEn="Description" valueAr={form.pricing.description.ar} valueEn={form.pricing.description.en} onChangeAr={(value) => updateLocalizedSection("pricing", "description", "ar", value)} onChangeEn={(value) => updateLocalizedSection("pricing", "description", "en", value)} textarea />
            </TabsContent>
            ...
            <TabsContent value="seo" className="space-y-4 pt-4">
              <LocalizedField labelAr="الوصف الافتراضي" labelEn="Default description" valueAr={form.defaultDescriptionAr} valueEn={form.defaultDescriptionEn} onChangeAr={(value) => updateField("defaultDescriptionAr", value)} onChangeEn={(value) => updateField("defaultDescriptionEn", value)} textarea />
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="keywordsAr">{isAr ? "كلمات مفتاحية عربية" : "Arabic keywords"}</Label>
                  <Textarea id="keywordsAr" value={keywordsAr} onChange={(event) => setKeywordsAr(event.target.value)} rows={5} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="keywordsEn">{isAr ? "كلمات مفتاحية إنجليزية" : "English keywords"}</Label>
                  <Textarea id="keywordsEn" value={keywordsEn} onChange={(event) => setKeywordsEn(event.target.value)} rows={5} />
                </div>
              </div>
            </TabsContent>
          </Tabs>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isAr ? "لقطة تقريبية لما سيظهر في الصفحات العامة." : "A rough glance at what will appear across public pages."}
          </CardDescription>
        </CardHeader>
...
function PreviewBlock({ title, badge, heading, description }: { title: string; badge: string; heading: string; description: string }) {
  return (
    <div className="rounded-2xl border p-4">
      <p className="mb-2 text-xs uppercase tracking-[0.2em] text-muted-foreground">{title}</p>
