...
const LocationPickerMap = dynamic(() => import("@/components/maps/location-picker-map"), {
  ssr: false,
  loading: () => <div className="bg-muted/30 h-[280px] w-full rounded-md border" />
});
...
export default function AttendanceSettingsClient({ locale: _locale }: { locale: Locale }) {
  ...
  const loadAll = React.useCallback(async () => {
    ...
    try {
      ...
      if (pRes.ok) {
        ...
        toast.error(
          pJson?.error ?? pickText(locale, t.attendance.loadPolicyFailed, "Failed to load policy.")
        );
      }
      ...
      if (lRes.ok) {
        ...
        toast.error(
          lJson?.error ??
            pickText(locale, t.attendance.loadLocationsFailed, "Failed to load locations.")
        );
      }
    } finally {
      setLoading(false);
  ...
  async function savePolicy() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(
          json?.error ?? pickText(locale, t.attendance.savePolicyFailed, "Failed to save policy.")
        );
        return;
      }
  ...
  async function createLocation() {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(
          json?.error ?? pickText(locale, "تعذر إنشاء الموقع.", "Failed to create location.")
        );
        return;
      }
  ...
  async function toggleActive(id: string, current: boolean) {
    ...
    if (!res.ok) {
      const json = await res.json().catch(() => null);
      toast.error(json?.error ?? pickText(locale, t.attendance.updateFailed, "Failed to update."));
      return;
    }
    await loadAll();
    toast.success(
  ...
  return (
    <>
      <div className="space-y-6">
        <div>
          <h3 className="text-start text-lg font-medium">
              ...
              <>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between rtl:lg:flex-row-reverse">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(v) =>
                      setPolicy((p) => (p ? { ...p, allowCheckInWithoutLocation: v } : p))
                    }
                  />
                </div>
                ...
                <div className="grid gap-2">
                  <Label className="text-start">
                    {pickText(
                      locale,
                      "الحد الأقصى لدقة الموقع (متر)",
                      "Max Location Accuracy (meters)"
                    )}
                  </Label>
                  <Input
                    inputMode="numeric"
          ...
          <CardHeader className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between rtl:lg:flex-row-reverse">
            ...
            <Dialog open={openCreate} onOpenChange={setOpenCreate}>
              <DialogTrigger asChild>
                <Button variant="default">{pickText(locale, "إضافة موقع", "Add Location")}</Button>
              </DialogTrigger>
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid gap-2">
                    <div className="flex items-center justify-between gap-2 rtl:flex-row-reverse">
                      <Label className="text-start">
                        {pickText(locale, "حدد الموقع من الخريطة", "Pick on map")}
                      </Label>
                      <Button
                        type="button"
                        variant="outline"
                        ...
                        onClick={() => {
                          if (!("geolocation" in navigator)) {
                            toast.error(
                              pickText(
                                locale,
                                t.attendance.geolocationNotSupported,
                          ...
                          navigator.geolocation.getCurrentPosition(
                            ...
                            () => {
                              toast.error(
                                pickText(
                                  locale,
                                  t.attendance.failedGetLocation,
                  ...
                  <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                    <div className="grid gap-2">
                      <Label className="text-start">
                        {pickText(locale, "خط العرض (lat)", "Latitude")}
                      </Label>
                      <Input
                        inputMode="decimal"
                        value={form.lat}
                    ...
                    <div className="grid gap-2">
                      <Label className="text-start">
                        {pickText(locale, "خط الطول (lng)", "Longitude")}
                      </Label>
                      <Input
                        inputMode="decimal"
                        value={form.lng}
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-start">
                        {pickText(locale, "الإحداثيات", "Coordinates")}
                      </TableHead>
                      <TableHead className="text-start">
                        {pickText(locale, "نصف القطر", "Radius")}
                      </TableHead>
                      <TableHead className="text-start">
                        {pickText(locale, t.common.status, "Status")}
                      </TableHead>
                      <TableHead className="text-start">
                        {pickText(locale, t.common.actions, "Actions")}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                        ...
                        <TableCell className="text-start">
                          {loc.isActive ? (
                            ...
                            <Badge variant="secondary">
                              {pickText(locale, "غير مفعل", "Inactive")}
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell className="text-start">
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pickText(locale, t.common.confirmDeleteTitle, "Confirm Deletion")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {pickText(
                locale,
                `هل أنت متأكد من حذف موقع «${locationToDelete?.nameAr ?? locationToDelete?.name}»؟`,
