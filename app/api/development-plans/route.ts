...
const developmentPlanSchema = z.object({
  ...
  status: z
    .enum(["DRAFT", "PENDING_APPROVAL", "IN_PROGRESS", "COMPLETED", "CANCELLED"])
    .optional()
    .default("DRAFT"),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = await checkRateLimit(request, {
      keyPrefix: "api:dev-plans:create",
      limit,
      windowMs: 60 * 1000
    });
    if (!rl.allowed) {
    ...
    if (!allowedRoles.includes(session.user.role || "")) {
      return NextResponse.json({ error: "لا تملك صلاحية إنشاء خطط تطوير" }, { status: 403 });
    }
    ...
    if (employee.user?.id) {
      const empName = [
        plan.employee.firstNameAr ?? plan.employee.firstName,
        plan.employee.lastNameAr ?? plan.employee.lastName
      ]
        .filter(Boolean)
        .join(" ");
      sendNotification({
    ...
    return NextResponse.json({ error: "حدث خطأ في إنشاء خطة التطوير" }, { status: 500 });
  }
}
