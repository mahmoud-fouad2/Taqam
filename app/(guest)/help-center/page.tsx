...
const helpTopics = [
  {
    ...
    descEn:
      "Set up the company, add employees, and configure departments and shifts in a clear path."
  },
  {
    icon: Clock3,
...
const setupSteps = [
  {
    ar: "أنشئ الشركة أو انتظر تفعيلها من فريق طاقم.",
    en: "Create the company or wait for it to be activated by the Taqam team."
  },
  {
    ar: "فعّل حساب مدير الشركة من رابط التفعيل المرسل إلى البريد.",
    en: "Activate the tenant admin account from the email activation link."
  },
  {
    ar: "أضف المستخدمين الأساسيين وحدد الأدوار المناسبة لهم.",
    en: "Invite the core users and assign the right roles."
  },
  {
    ar: "أدخل الموظفين، الأقسام، المسميات الوظيفية، والورديات.",
    en: "Add employees, departments, job titles, and shifts."
  },
  {
    ar: "ابدأ التشغيل اليومي ثم راجع التقارير والطلبات من لوحة التحكم.",
    en: "Start daily operations, then review reports and requests from the dashboard."
  }
];
...
const quickLinks = [
  {
    ...
    descEn:
      "The right starting point to understand the platform and get daily operations configured."
  },
  {
    icon: MessageSquare,
    href: "/faq",
          ...
          <div className="container mx-auto px-4">
            <div className="grid items-center gap-12 lg:grid-cols-2">
              <div className="text-center lg:text-start">
                ...
                <p className="text-muted-foreground/80 mx-auto mt-5 max-w-lg text-lg leading-relaxed lg:mx-0">
                  {isAr
              ...
              <div className="relative mx-auto hidden w-full max-w-md lg:block">
                ...
                <div className="pointer-events-none absolute -end-10 -bottom-10 -z-10 h-40 w-40 rounded-full bg-blue-400/20 blur-3xl" />

                <div className="dark:bg-card/90 relative overflow-hidden rounded-tl-[4rem] rounded-tr-3xl rounded-br-[4rem] rounded-bl-3xl border-8 border-white/50 bg-white/90 shadow-2xl dark:border-white/10">
                ...
                {/* Floating badge */}
                <div className="dark:bg-card/95 absolute -start-6 bottom-16 rounded-2xl border border-white/60 bg-white/95 p-3.5 shadow-xl backdrop-blur-md dark:border-white/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-foreground text-sm font-bold">
                        {isAr ? "مكتبة شاملة" : "Comprehensive Library"}
                      </p>
                      <p className="text-muted-foreground text-xs font-medium">
                        {isAr ? "أدلة في جميع المجالات" : "Guides in all areas"}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {quickLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    <div className="bg-primary/5 group-hover:bg-primary/10 absolute -end-8 -top-8 -z-10 h-32 w-32 rounded-full transition-transform duration-500 group-hover:scale-[2.5]" />
        ...
        <section id="guides" className="relative border-t py-28">
          ...
          <div className="container mx-auto px-4">
            <div className="mb-16 max-w-2xl">
              <span className="border-primary/20 bg-primary/[0.07] text-primary mb-4 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-semibold">
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {helpTopics.map((topic) => {
                ...
                return (
                  <div
                    key={topic.titleEn}
                    className="group border-border/40 bg-card/60 hover:border-border/80 hover:bg-card/95 relative flex cursor-default flex-col gap-4 overflow-hidden rounded-[2.5rem] border p-8 shadow-sm backdrop-blur-md transition-all duration-500 hover:-translate-y-1 hover:shadow-xl">
                    <div
                      className={`flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-gradient-to-br shadow-inner ${topic.color}`}>
            ...
            <div className="border-border/50 bg-card/90 mt-16 overflow-hidden rounded-[2.5rem] border shadow-sm backdrop-blur-sm">
              <div className="border-b px-8 py-6">
                <div className="flex items-center gap-3">
                  <div className="bg-primary/[0.07] flex h-10 w-10 items-center justify-center rounded-xl">
                    <PlugZap className="text-primary h-5 w-5" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold">
                      {isAr ? "مسار الإعداد السريع" : "Quick setup path"}
                    </h3>
                    <p className="text-muted-foreground text-sm">
                      {isAr
                        ? "الخطوات الأساسية من تفعيل الحساب حتى بدء التشغيل اليومي."
                        : "The core steps from account activation to daily operations."}
                    </p>
                  </div>
                </div>
              </div>
              <ol className="divide-border/50 divide-y">
                {setupSteps.map((step, index) => (
                  <li
                    key={index}
                    className="hover:bg-muted/30 flex items-start gap-5 px-8 py-5 text-sm transition-colors">
                    <span className="bg-primary/10 text-primary mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold">
                      {index + 1}
                    </span>
                    <span className="text-foreground/80 leading-7">{isAr ? step.ar : step.en}</span>
                  </li>
                ))}
              </ol>
            </div>
        ...
        <section className="border-t py-24">
          <div className="container mx-auto px-4">
            <div className="border-border/50 bg-card/90 mx-auto max-w-3xl rounded-[2.5rem] border p-10 text-center shadow-sm backdrop-blur-sm sm:p-14">
              <div className="bg-primary/[0.07] mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl">
