...
> = {
  ...
  indigo: {
    section:
      "bg-gradient-to-b from-indigo-50/60 via-background to-background dark:from-indigo-950/20",
    badge:
...
export function MarketingPageHero({
  ...
  return (
    <section
      className={cn(
        "relative overflow-hidden border-b pt-20 pb-20 sm:pt-28",
        styles.section,
        className
      )}>
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div
          className={cn(
            "absolute start-1/4 top-8 h-64 w-64 rounded-full blur-[100px]",
            styles.orbPrimary
          )}
        />
        <div
          className={cn(
            "absolute end-1/4 top-16 h-56 w-56 rounded-full blur-[80px]",
            styles.orbSecondary
          )}
        />
      </div>
      ...
      <div className="container mx-auto px-4">
        <div className="mx-auto max-w-4xl text-center">
          ...
          {badge ? (
            <span
              className={cn(
                "inline-flex rounded-full border px-4 py-1.5 text-xs font-semibold",
                styles.badge
              )}>
              {badge}
            </span>
          ...
          <p className="text-muted-foreground/80 mx-auto mt-5 max-w-2xl text-lg leading-8">
            {description}
          </p>
          ...
          {stats.length > 0 ? (
            <div
              className={cn(
                "border-border/50 bg-card/80 mx-auto mt-12 grid gap-4 rounded-[2rem] border p-7 shadow-sm backdrop-blur-sm",
              ...
              {stats.map((stat) => (
                <div key={`${stat.value}:${stat.label}`}>
                  <div className="text-primary text-3xl font-extrabold">{stat.value}</div>
                  <p className="text-muted-foreground/80 mt-1.5 text-sm">{stat.label}</p>
                </div>
              ))}
            </div>
          ) : null}
